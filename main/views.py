...
from main.models import CreateExam
#, Answer, ExamAnswerSheet, SectionAnswerSheet
#, McqAnswerToMcqOption
...
def public_exam_list(request):
	context_dict = {"exam_list":CreateExam.objects.filter(owner__isnull=True)}
...
def user_exam_list(request,username):
	...
	context_dict = {"exam_list":CreateExam.objects.filter(owner=user)}
...
def exam_cover(request,eid):
	exam = get_object_or_404(CreateExam,id=eid)
