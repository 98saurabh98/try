...
class Option(models.Model):
	...
	def add_option_here(obj,optionname):
		

		if UNKNOWN_TAG_ACTION=="create":
#			option_here = Option(name=optionname)
			option_here.save()
			obj.add_options.add(option_here)
		elif UNKNOWN_TAG_ACTION=="error":
			raise
...
#	exam = models.ForeignKey(CreateExam)
...
#	class McqOption(models.Model):          (added extra hash)
	title = models.CharField(max_length=30,blank=True)
	text = models.TextField(blank=False)
	add_options = models.ManyToManyField(Option)
	Add_To_Section = models.ManyToManyField(Section)
#	sno = models.PositiveIntegerField(default=0)
...
#	class McqAnswer(models.Model, SpecialAnswer): (added extra hash)
...
#------------------Try--------#------------------Try--------One extra tab added------------------------------------------------------------------------------------------------------------------------

#	class McqAnswerToMcqOption(models.Model):         (added extra hash)
...
class MakeExamLive(models.Model):
	exam = models.ForeignKey(CreateExam)
	...
	def get_timer_status(self):
		if self.start_time==None:
			if self.end_time==None:	return MakeExamLive.TIMER_NOT_SET
			else: return MakeExamLive.TIMER_ERROR
		now = timezone.now()
		if self.end_time==None:
			if now < self.start_time:
				return MakeExamLive.TIMER_NOT_STARTED
			else:
				return MakeExamLive.TIMER_IN_PROGRESS
		if self.end_time < self.start_time:
			return MakeExamLive.TIMER_ERROR
		if now < self.start_time:
			return MakeExamLive.TIMER_NOT_STARTED
		elif now < self.end_time:
			return MakeExamLive.TIMER_IN_PROGRESS
		else:
			return MakeExamLive.TIMER_ENDED

	def get_timer_status_str(self):
		timer_status = self.get_timer_status()
		try:
			result_str = MakeExamLive.STATUS_STR_DICT[timer_status]
			if(timer_status==MakeExamLive.TIMER_NOT_STARTED):
	...
	def get_attempt_duration(self):
		...
		"""
		timer_status = self.get_timer_status()
		if timer_status in MakeExamLive.DURATIONLESS_TIMER_STATII:
			return None
		if timer_status == MakeExamLive.TIMER_IN_PROGRESS:
